...
func TestEditPasswordCmd(t *testing.T) {
	cfg, err := config.New()
	...
	data := []testData{
		{
			name: "success",
			args: testArgs{
				input:        []string{"yandex.disk", "-p new-password"},
				name:         "yandex.disk",
				newName:      "",
	...
	for _, tt := range data {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mockUsecase := NewMockEditPassowrdUsecase(t)
			...
			defer func() {
				...
				cmd := NewEditPasswordCmd(mockUsecase, cfg, lg)
			...
			mockUsecase.EXPECT().EditPassword(mock.Anything, tt.args.name, tt.args.newName, tt.args.newPassoword, tt.args.lockPassword).Return(tt.expected.usecaseError)
			if tt.expected.usecaseError != nil {
