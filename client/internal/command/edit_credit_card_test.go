...
func TestEditCreditCardCmd(t *testing.T) {
	cfg, err := config.New()
	...
	for _, tt := range data {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockUsecase := NewMockEditCreditCardUsecase(t)
			...
			defer func() {
				...
				cmd := NewEditCreditCardCmd(mockUsecase, cfg, lg)
			...
			mockUsecase.EXPECT().EditCard(mock.Anything, tt.args.name, tt.args.newName, tt.args.newCard, tt.args.lockPassword).Return(tt.expected.usecaseError)
			if tt.expected.usecaseError != nil {
