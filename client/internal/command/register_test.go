...
func TestRegisterCmd(t *testing.T) {
	...
	for _, tt := range data {
		...
		t.Run(tt.name, func(t *testing.T) {
			registerMock := NewMockRegisterUsecase(t)
			result := new(bytes.Buffer)
			errors := new(bytes.Buffer)

			defer func() {
				cmd := NewRegisterCmd(registerMock, cfg, lg)
			...
			registerMock.EXPECT().Registration(mock.Anything, tt.args.login, tt.args.password).Return(tt.expected.usecaseError)
			if tt.expected.usecaseError != nil {
