...
func TestGetPasswordCmd(t *testing.T) {
	...
	for _, tt := range data {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockUsecase := NewMockGetPasswordUsecase(t)
			...
			defer func() {
				...
				cmd := NewGetPasswordCmd(mockUsecase, cfg, lg)
			...
			mockUsecase.EXPECT().GetPasswordByName(mock.Anything, tt.args.name, tt.args.lockPassword).Return(tt.expected.password, tt.expected.usecaseError)
			if tt.expected.usecaseError != nil {
